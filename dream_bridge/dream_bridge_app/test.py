...
class DreamFormEdgeCaseTest(TestCase):
    """
    Teste les cas limites du formulaire
    DreamForm (données invalides, valeurs inattendues).
    """
    def setUp(self):
    ...
    def test_form_invalid_emotion(self):
        """Le formulaire doit être invalide si
        l'émotion n'est pas dans la liste."""
        form_data = {
...
class DreamStatusApiErrorTest(TestCase):
    ...
    def test_api_invalid_id(self):
        ...
        url = reverse(
            ...
            kwargs={'dream_id': fake_id}
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, 404)

    def test_api_permission_denied(self):
        """L'API doit retourner 404 si l'utilisateur n'a
        pas accès au rêve."""
...
class DreamFormTest(TestCase):
    ...
    Vérifie que le formulaire accepte et
    rejette correctement les données.
    """
    def setUp(self):
        self.user = User.objects.create_user(
            username='dreamformuser',
            password='password'
        )

    def test_form_valid_data(self):
        """Le formulaire doit être valide avec
        un fichier audio correct."""
        from django.core.files.uploadedfile import SimpleUploadedFile
    ...
    def test_form_missing_audio(self):
        """Le formulaire doit être invalide si le
        fichier audio est manquant."""
        form_data = {}
        form = DreamForm(data=form_data)
        self.assertFalse(form.is_valid())
    ...
    def test_form_audio_too_large(self):
        """Le formulaire doit être invalide si le
        fichier audio dépasse la taille max."""
...
class UserProfileModelTest(TestCase):
    """
    Teste le modèle UserProfile pour s'assurer que les
    propriétés fonctionnent correctement.
    """
...
class DreamStatusApiTest(TestCase):
    """
    Teste l'API check_dream_status_api pour s'assurer qu'elle
...
class MetricsDashboardLogicTest(TestCase):
    """
    Teste les fonctions de calcul du tableau de bord
    de manière isolée
    """
    def setUp(self):
        """
        ...
        # Un rêve pour un autre utilisateur, pour s'assurer qu'il n'est
        # pas compté
        other_user = User.objects.create_user(
            username='otheruser',
            password='password'
    ...
    def test_total_dreams_calculation(self):
        """Vérifie que la fonction total_dreams compte correctement les
        rêves de l'utilisateur."""
...
class DreamAppViewsTest(TestCase):
    ...
    def setUp(self):
        ...
        dream1 = Dream.objects.create(
            ...
            status='COMPLETED',
            emotion='joie',
            transcription="Un beau rêve."
        )
        dream1.generated_image.save(
            'test1.gif',
            SimpleUploadedFile('test1.gif', fake_image_content, 'image/gif')
        )
        dream2 = Dream.objects.create(
            user=self.user,
            status='COMPLETED',
            emotion='peur',
            transcription="Un cauchemar."
        )
        dream2.generated_image.save(
            'test2.gif',
            SimpleUploadedFile('test2.gif', fake_image_content, 'image/gif')
        )
...
class DreamCreateViewIntegrationTest(TestCase):
    ...
    def test_post_audio_creates_dream_and_starts_task(
            self,
            mock_celery_task_delay,
    ):
...
class ServicesLogicTest(TestCase):
    """
    Teste la fonction d'orchestration `orchestrate_dream_generation`
    en simulant
    les appels aux API externes (Groq, Mistral) pour ne pas
    dépendre d'eux.
    """
    def setUp(self):
        self.user = User.objects.create_user(
    ...
    def test_orchestrate_dream_generation_success(
        self,
        mock_get_emotion,
        mock_groq,
        mock_mistral,
        ...
        # On configure le comportement de nos mocks pour qu'ils retournent
        # des fausses données.
        # Transcription simulée
        mock_audio = mock_groq.return_value.audio
        mock_transcriptions = mock_audio.transcriptions
        mock_create = mock_transcriptions.create
        mock_create.return_value.text = "Ceci est une transcription simulée."

        # Réponse simulée pour la génération de prompt
        mock_chat = mock_groq.return_value.chat
        mock_completions = mock_chat.completions
        mock_choices = mock_completions.create.return_value.choices
        mock_choices[0].message.content = "Un prompt d'image simulé."
        ...
        mock_files = mock_mistral.return_value.files
        mock_download_obj = mock_files.download.return_value
        mock_read = mock_download_obj.read
        fake_image_data = b'fausses_donnees_image'
        mock_read.return_value = fake_image_data

        # --- 2. Act (Action) ---
        # On appelle la fonction que l'on veut tester.
        orchestrate_dream_generation(
            str(self.dream.id),
            'fake/path/to/audio.webm'
        )

        # --- 3. Assert (Vérification) ---
    def test_dashboard_view_authenticated(self):
        """Vérifie que le dashboard s'affiche et contient
        les bonnes données."""
    ...
    def test_orchestrate_dream_generation_api_failure(self, mock_groq):
        ...
        mock_create = mock_groq.return_value.audio.transcriptions.create
        mock_create.side_effect = Exception("Erreur API simulée")
        ...
        self.assertIn(
            ...
            "[Errno 2] No such file or directory: 'fake/path/to/audio.webm'",
            self.dream.error_message
        )


class SecurityTest(TestCase):
    """Vérifie que les utilisateurs ne peuvent pas accéder
    ...
    def test_user_cannot_access_other_user_dream_status(self):
        """
        Vérifie qu'un utilisateur connecté ne peut pas voir
        la page de statut d'un rêve qui ne lui appartient pas.
        """
        # On connecte user2
        ...
        url = reverse(
            'dream_bridge_app:dream-status',
            kwargs={'dream_id': self.dream_user1.id}
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, 404)
...
class DreamModelTest(TestCase):
    ...
    Permet de s'assurer que la logique métier du modèle fonctionne
    comme attendu.
    """
    def setUp(self):
        self.user = User.objects.create_user(
    ...
    def test_get_emotion_display(self):
        """
        Vérifie que get_emotion_display retourne le label correct pour
        l'émotion. Permet d'afficher le bon texte dans les templates.
        """
        dream = Dream.objects.create(
            user=self.user,
            status='COMPLETED',
...
class ServicesTest(TestCase):
    ...
    def test_get_emotion_from_text(self):
        ...
        On s'attend à ce que l'émotion retournée soit dans la liste
        des émotions connues.
        """
        text = "C'était une journée merveilleuse et pleine de bonheur."
        emotion = get_emotion_from_text(text)
        self.assertIn(emotion, ['joie', 'tristesse',
                                'colère', 'peur', 'surprise',
                                'dégoût', 'neutre'])
...
class TasksTest(TestCase):
    ...
    On vérifie que la fonction d'orchestration est appelée et que le fichier
    temporaire est supprimé.
    """
    @patch('dream_bridge_app.tasks.orchestrate_dream_generation')
    @patch('os.remove')
    def test_process_dream_audio_task_deletes_temp_file(
            self, mock_remove, mock_orchestrate):
        """
        Vérifie que le fichier temporaire est supprimé après traitement.
        On mocke orchestrate_dream_generation pour ne pas exécuter
        la vraie logique.
        """
